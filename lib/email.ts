...
export async function sendInternalLeadNotification(
  ...
  const from = process.env.EMAIL_FROM ?? "leads@brizasolutions.com";
...
export async function sendLeadConfirmationEmail(
  ...
  const client = getResendClient();
  const from = process.env.EMAIL_FROM ?? "hello@brizasolutions.com";
  ...
  const body =
    lead.source === "contact-form"
      ? `<p>Thanks for reaching out. A member of the Briza Solutions team will review your message and get back to you shortly.</p>`
      : `<p>Thanks for telling us about ${lead.businessName}. A member of the Briza
         Solutions team will review your ${lead.industryLabel.toLowerCase()} details
         and reach out shortly to talk through a plan.</p>`;

  await client.emails.send({
    from,
    to: lead.email,
    subject: "We've got your info — Briza Solutions",
    html: `
      <p>Hi ${firstName},</p>
      ${body}
      <p>— The Briza Solutions team</p>
    `,
  });
}
